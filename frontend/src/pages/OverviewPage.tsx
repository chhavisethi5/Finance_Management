...
export default function OverviewPage() {
  ...
  const { openProfileModal } = useOutletContext<{ openProfileModal: () => void }>();
  const [status, setStatus] = useState<BudgetStatus | null>(null);
  const isBaselineUnset = currentUser && (currentUser.manual_savings_offset === 0 || currentUser.manual_savings_offset === null);
  ...
  return (
    <>
      ...
      {isBaselineUnset && (
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-500/20 bg-amber-500/5 p-3.5 text-xs text-amber-300">
          <div className="flex items-center gap-2">
            <AlertTriangle className="h-4 w-4 shrink-0 text-amber-400" />
            <span>
              Your pre-existing liquid savings offset and baseline numbers are currently set to zero. Add your existing savings to get accurate net worth & emergency fund progress metrics.
            </span>
          </div>
          <button
            onClick={openProfileModal}
            className="text-xs font-semibold text-[#4f8ef7] hover:underline whitespace-nowrap"
          >
            Configure Now →
          </button>
        </div>
      ...
      {status && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
          <StatCard
            ...
            editTooltip="Update your monthly income and profile settings"
          />
          <StatCard
          ...
          <StatCard
            ...
            badge="Rollover"
            onEdit={openProfileModal}
            editTooltip="Manage your pre-existing savings and baseline numbers"
          />
          <StatCard
